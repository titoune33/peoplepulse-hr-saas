...
export function HRISStatus() {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
      <span className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mr-2">Connected HRIS:</span>
      {connections.map((c) => (
        <div
          key={c.provider}
        ...
        >
          ...
          <span className="text-[10px] text-emerald-500 dark:text-emerald-400">· synced {c.lastSync}</span>
        </div>
      ))}
