...
const severityConfig = {
  high: { icon: AlertTriangle, color: "text-red-600 dark:text-red-400", bg: "bg-red-50 dark:bg-red-950/50", border: "border-red-200 dark:border-red-800", label: "High Priority" },
  medium: { icon: Info, color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-50 dark:bg-amber-950/50", border: "border-amber-200 dark:border-amber-800", label: "Medium Priority" },
  low: { icon: ThumbsUp, color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50 dark:bg-emerald-950/50", border: "border-emerald-200 dark:border-emerald-800", label: "Low Priority" },
};

const categories = [
  { key: "All", label: "All Insights", icon: Zap },
  { key: "high", label: "High Priority", icon: AlertTriangle },
  { key: "medium", label: "Medium", icon: Info },
  { key: "low", label: "Positive Signals", icon: ThumbsUp },
];
...
export default function InsightsPage() {
  ...
  const fetchInsights = useCallback(async () => {
    ...
    try {
      ...
      setError("Backend unavailable. Showing demo insights.");
    } finally {
      setIsLoading(false);
    }
  }, [token]);
    ...
    <div className="min-h-full">
      ...
      <div className="p-6 max-w-[1000px] mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">AI Insights</h1>
            <p className="text-sm text-zinc-500 mt-0.5">
              {highCount} high · {mediumCount} medium · {insights.length} total
        ...
        {filtered.length === 0 ? (
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-12 text-center">
            ...
            <p className="text-sm font-medium text-zinc-600 dark:text-zinc-400 mb-1">No insights for this category</p>
            <p className="text-xs text-zinc-500 dark:text-zinc-500">Try selecting a different filter</p>
          </div>
        ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-5">
            ...
            <p className="text-xs text-zinc-500 mt-0.5">Updated every 24 hours</p>
          </div>
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-5">
            <div className="flex items-center gap-2 mb-2">
              <TrendingUp className="w-4 h-4 text-emerald-500" />
              <span className="text-xs font-medium text-zinc-500 uppercase tracking-wider">Data Sources</span>
            </div>
            <p className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">147 employees</p>
            <p className="text-xs text-zinc-500 mt-0.5">Across BambooHR, Paylocity, Workday</p>
          </div>
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-5">
            <div className="flex items-center gap-2 mb-2">
              <Zap className="w-4 h-4 text-amber-500" />
              <span className="text-xs font-medium text-zinc-500 uppercase tracking-wider">Last Analysis</span>
            </div>
            <p className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">{token ? "Just now" : "2 hours ago"}</p>
            <p className="text-xs text-zinc-500 mt-0.5">{insights.length} insights generated</p>
          </div>
        </div>
      </div>
    </div>
